...
func retry(attempts int, sleep time.Duration, f func() error) (err error) {
	...
	for i := 0; i < attempts; i++ {
		...
		log.Printf("Running exec.Command() and waiting for it to finish...")
		err = cmd.Run()
		// error handling if error is not nill
		// if the error is nil, return nill
		if err == nil {
			//
			log.Printf("there was no error, so return nill.")
			// return nil
			return nil
		}
	}

	// this last return function handles the error finally.
